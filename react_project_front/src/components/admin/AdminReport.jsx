...
const AdminReport = ({
  ...
  setIsModalOpen,
  isModalOpen,
  boardDetail, // CommunityDetail에 넘길 게시글/댓글 데이터
  selectedReport,
  setSelectedReport,
  showDetail,
  setShowDetail,
  detailRef, // CommunityDetail 영역 DOM 참조 (스크롤용)
  reportAction,
  changeReportAction,
  handleSubmit,
}) => {
  return (
    <>
      <div className={styles.report_wrap}>
        ...
        <div className={styles.report_table}>
          <table>
            ...
            <tbody>
              {/* reportList 배열을 순회하며 각 신고를 행으로 표시 */}
      ...
      {isModalOpen && (
        <div className={styles.modal_bg} onClick={() => setIsModalOpen(false)}>
          <div
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className={styles.action_wrap}>
                ...
                <div className={styles.action_section}>
                  ...
                  <div className={styles.action_row}>
                    <span>미처리</span>
                    <input type="radio" name="boardAction" value="미처리" checked={reportAction.boardAction === "미처리"} onChange={changeReportAction} />
                  </div>
                  <div className={styles.action_row}>
                    <span>비공개 처리</span>
                    <input type="radio" name="boardAction" value="비공개 처리" checked={reportAction.boardAction === "비공개 처리"} onChange={changeReportAction} />
                  </div>
                  <div className={styles.action_row}>
                    <span>삭제 처리</span>
                    <input type="radio" name="boardAction" value="삭제 처리" checked={reportAction.boardAction === "삭제 처리"} onChange={changeReportAction} />
                  </div>
                </div>
                ...
                <div className={styles.action_section}>
                  ...
                  <div className={styles.action_row}>
                    <span>미처리</span>
                    <input type="radio" name="memberAction" value="미처리" checked={reportAction.memberAction === "미처리"} onChange={changeReportAction} />
                  </div>
                  <div className={styles.action_row}>
                    <span>경고 처리</span>
                    <input type="radio" name="memberAction" value="경고 처리" checked={reportAction.memberAction === "경고 처리"} onChange={changeReportAction} />
                  </div>
                  <div className={styles.action_row}>
                    <span>이용 정지 (1일)</span>
                    <input type="radio" name="memberAction" value="1" checked={reportAction.memberAction === "1"} onChange={changeReportAction} />
                  </div>
                  <div className={styles.action_row}>
                    <span>이용 정지 (3일)</span>
                    <input type="radio" name="memberAction" value="3" checked={reportAction.memberAction === "3"} onChange={changeReportAction} />
                  </div>
                  <div className={styles.action_row}>
                    <span>이용 정지 (7일)</span>
                    <input type="radio" name="memberAction" value="7" checked={reportAction.memberAction === "7"} onChange={changeReportAction} />
                  </div>
                  <div className={styles.action_row}>
                    <span>영구정지</span>
                    <input type="radio" name="memberAction" value="영구정지" checked={reportAction.memberAction === "영구정지"} onChange={changeReportAction} />
                  </div>
                </div>
              </div>
              ...
              <div className={styles.modal_btn_wrap}>
                ...
                <button
                  ...
                  onClick={() => setShowDetail(true)} // showDetail true로 바꾸면 Page의 useEffect가 감지해서 스크롤
                >
                  원본
                </button>
              </div>
